...
exports.run = async (client, message) => {
	...
	.addField(`Doğru Kullanım:`, `${ayarlar.prefix}reklam aç veya kapat`)
	if(secenekler.length < 1) return message.channel.send(errembed);
	//if(secenekler === "aç" || "kapat") return message.channel.send(errembed);

	if (secenekler === "aç") {
		message.channel.send(`Reklam engelleme: **açık**!`)
  ...
  process.exit(1);
	...
	if (secenekler === "kapat") {
		message.channel.send(`Reklam engelleme: **kapalı**!`)
  let linkEngel = JSON.parse(fs.readFileSync("././jsonlar/linkEngelle.json", "utf8"));
  if(!linkEngel[message.guild.id]){
		linkEngel[message.guild.id] = {
			linkEngel: "kapali"
