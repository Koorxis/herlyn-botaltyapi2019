...
exports.run = async (client, msg, args) => {
    if (!args[0]) {
        return msg.channel.send('Lütfen seçimini yap taş, kağıt yada makas & hs!tkm <taş,kağıt,makas>');
    }
    ...
    if (result === 'won') {
        answer = ':trophy: Başarılı, sen **Kazandın** :trophy: \nSenin Seçtiği: `' + choice + '` | Benim Seçtiğim: `' + botChoice + '`';
    } else if (result === 'lost') {
        answer = ':x: Bidakine **Kaybetin Dostum** :x: \nSenin Seçtiğin: `' + choice + '` | Benim Seçtiğim: `' + botChoice + '`';
    } else if (result === 'draw') {
        answer = ':neutral_face: **Berabere** :neutral_face:\nSenin Seçimin: `' + choice + '` | Benim Seçtiğim: `' + botChoice + '`';
    }
